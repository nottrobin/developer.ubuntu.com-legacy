...
APT_LINES = [
    (
        "deb http://ppa.launchpad.net/ci-train-ppa-service/"
        "stable-phone-overlay/ubuntu %s main"
    ) % RELEASE,
...
def download_packages():
    ...
    for pkg in PACKAGES:
        ...
        if call_with_apt_args(
            "apt-get download --allow-unauthenticated %s" % pkg
        ) == 0:
            ...
            print(
                "Exec: %s" % " ".join(
                    ["dpkg-deb", "--extract", deb_file, OUTPUT_DIR]
                )
            )
            if subprocess.call(
                ["dpkg-deb", "--extract", deb_file, OUTPUT_DIR]
